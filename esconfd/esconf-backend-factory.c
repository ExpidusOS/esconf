...
#ifdef BUILD_ESCONF_BACKEND_PERCHANNEL_XML
...
static void
esconf_backend_factory_ensure_backends(void)
{
    if(backends)
        return;
...
#ifdef BUILD_ESCONF_BACKEND_PERCHANNEL_XML
    {
        ...
        *gtype = ESCONF_TYPE_BACKEND_PERCHANNEL_XML;
        g_hash_table_insert(backends,
                            (gpointer)ESCONF_BACKEND_PERCHANNEL_XML_TYPE_ID,
                            gtype);
    }
#endif
}
...
EsconfBackend *
esconf_backend_factory_get_backend(const gchar *type,
                                   GError **error)
{
    EsconfBackend *backend = NULL;
    GType *backend_gtype;
    
    esconf_backend_factory_ensure_backends();
    
    backend_gtype = g_hash_table_lookup(backends, type);
    if(!backend_gtype) {
        if(error) {
            g_set_error(error, ESCONF_ERROR, 0,
                        _("Unable to find Esconf backend of type \"%s\""),
                        type);
        }
        return NULL;
    }
    ...
    if(!esconf_backend_initialize(backend, error)) {
...
void
esconf_backend_factory_cleanup (void)
{
  if(backends) {
