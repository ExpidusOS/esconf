...
static void
esconf_ensure_group_cache(void)
{
...
static gboolean
esconf_user_is_in_group(const gchar *user,
                        const gchar *group)
{
    GHashTable *members;
    
    esconf_ensure_group_cache();
...
gboolean
esconf_user_is_in_list(const gchar *list)
{
    gboolean ret = FALSE;
    ...
    for(i = 0; tokens[i]; ++i) {
        if(!*tokens[i])
            ...
            if(esconf_user_is_in_group(user_name, tokens[i]+1)) {
                ret = TRUE;
                break;
            }
        } else {
