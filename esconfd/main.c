...
static void
esconf_dbus_name_lost (GDBusConnection *connection,
                       const gchar     *name,
                       gpointer         user_data) {
    GMainLoop *main_loop;
     ...
     char **argv)
{
    ...
    GOptionEntry options[] = {
        { "version", 'V', G_OPTION_FLAG_IN_MAIN, G_OPTION_ARG_NONE, &print_version,
            N_("Prints the esconfd version."), NULL },
    ...
    esconfd = esconf_daemon_new_unique(backends, &error);
    if(!esconfd) {
        g_critical("Esconfd failed to start: %s\n", error->message);
        g_error_free(error);
        return EXIT_FAILURE;
    }
    g_strfreev(backends);
    ...
    if(do_daemon) {
        ...
        if(child_pid < 0) {
            ...
            fprintf(stdout, "ESCONFD_PID=%d; export ESCONFD_PID;", child_pid);
            exit(0);
        }
