...
/**
 * SECTION:esconf-binding
 * @title: Esconf-GObject Binding
 * @short_description: Functions to bind Esconf properties to GObject properties
 *
 * Often it may be useful to bind an Esconf property to a GObject property.
 * Settings dialogs often display the current value of an Esconf property,
 * and a user may edit the value to change the value in the Esconf store.
 * If the Esconf property changes outside the settings dialog, the user will
 * usually want to see the settings dialog automatically update to reflect
 * the new value.
 *
 * With a single line of code, Esconf's binding functionality can automate
 * all this.
 **/
...
static void esconf_g_property_object_notify(GObject *object,
                                            GParamSpec *pspec,
                                            gpointer user_data);
static void esconf_g_property_object_disconnect(gpointer user_data,
                                                GClosure *closure);
static void esconf_g_property_channel_notify(EsconfChannel *channel,
                                             const gchar *property,
                                             const GValue *value,
                                             gpointer user_data);
static void esconf_g_property_channel_disconnect(gpointer user_data,
                                                 GClosure *closure);
...
static void
esconf_g_property_object_notify_gdkcolor(EsconfGBinding *binding)
{
    ...
    esconf_channel_set_array(binding->channel, binding->esconf_property,
                             ESCONF_TYPE_UINT16, &color->red,
                             ESCONF_TYPE_UINT16, &color->green,
                             ESCONF_TYPE_UINT16, &color->blue,
                             ESCONF_TYPE_UINT16, &alpha,
                             G_TYPE_INVALID);
    g_signal_handler_unblock(G_OBJECT(binding->channel), binding->channel_handler);
}

static void
esconf_g_property_object_notify_gdkrgba(EsconfGBinding *binding)
{
...
static void
esconf_g_property_object_notify(GObject *object,
                                GParamSpec *pspec,
                                gpointer user_data)
{
    EsconfGBinding *binding = user_data;
    GValue src_val = { 0, };
    GValue dst_val = { 0, };

    g_return_if_fail(G_IS_OBJECT(object));
    g_return_if_fail(binding->object == object);
    g_return_if_fail(ESCONF_IS_CHANNEL(binding->channel));

    if(G_PARAM_SPEC_VALUE_TYPE(pspec) == __gdkcolor_gtype) {
        /* we need to handle this in a different way */
        esconf_g_property_object_notify_gdkcolor(binding);
        return;
    }

    if(G_PARAM_SPEC_VALUE_TYPE(pspec) == __gdkrgba_gtype) {
        /* we need to handle this in a different way */
        esconf_g_property_object_notify_gdkrgba(binding);
        return;
    }
    ...
    g_value_init(&dst_val, binding->esconf_property_type);
    if(g_value_transform(&src_val, &dst_val)) {
        g_signal_handler_block(G_OBJECT(binding->channel),
                               binding->channel_handler);
        esconf_channel_set_property(binding->channel,
                                    binding->esconf_property,
                                    &dst_val);
...
static void
esconf_g_property_object_disconnect(gpointer user_data,
                                    GClosure *closure)
{
    EsconfGBinding *binding = user_data;

    g_return_if_fail(G_IS_OBJECT(binding->object));
    g_return_if_fail(!binding->channel || ESCONF_IS_CHANNEL(binding->channel));
    ...
    g_free(binding->esconf_property);
    g_free(binding->object_property);
    g_slice_free(EsconfGBinding, binding);
}

static void
esconf_g_property_channel_notify_gdkcolor(EsconfGBinding *binding,
                                          const GValue *value)
{
    GPtrArray *arr;
...
static void
esconf_g_property_channel_notify_gdkrgba(EsconfGBinding *binding,
                                         const GValue *value)
{
    GPtrArray *arr;
    FakeGdkRGBA color = { 0, };
...
static void
esconf_g_property_channel_notify(EsconfChannel *channel,
                                 const gchar *property,
                                 const GValue *value,
                                 gpointer user_data)
{
   ...
   if(__gdkcolor_gtype == binding->esconf_property_type) {
       /* we need to handle this in a different way */
        esconf_g_property_channel_notify_gdkcolor(binding, value);
        return;
    }

    if(__gdkrgba_gtype == binding->esconf_property_type) {
       /* we need to handle this in a different way */
        esconf_g_property_channel_notify_gdkrgba(binding, value);
        return;
    }
...
static void
esconf_g_property_channel_disconnect(gpointer user_data,
                                     GClosure *closure)
{
    EsconfGBinding *binding = user_data;
...
static gulong
esconf_g_property_init(EsconfChannel *channel,
                       ...
                       GType object_property_type)
{
    EsconfGBinding *binding;
    gchar *detailed_signal;
    GValue value = { 0, };

    binding = g_slice_new(EsconfGBinding);
    binding->channel = channel;
    binding->esconf_property_type = esconf_property_type;
    binding->esconf_property = g_strdup(esconf_property);
    ...
    binding->object_handler = g_signal_connect_data(G_OBJECT(object),
                                                    detailed_signal,
                                                    G_CALLBACK(esconf_g_property_object_notify),
                                                    binding,
                                                    esconf_g_property_object_disconnect, 0);
    g_free(detailed_signal);

    /* transfer channel property to the object */
    if(esconf_channel_get_property(channel, esconf_property, &value)) {
        esconf_g_property_channel_notify(channel, esconf_property,
                                         &value, binding);
        g_value_unset(&value);
    }

    /* monitor channel for property changes */
    detailed_signal = g_strconcat("property-changed::", esconf_property, NULL);
    binding->channel_handler = g_signal_connect_data(G_OBJECT(channel),
                                                     detailed_signal,
                                                     G_CALLBACK(esconf_g_property_channel_notify),
                                                     binding,
                                                     esconf_g_property_channel_disconnect, 0);
    g_free(detailed_signal);
...
void
_esconf_g_bindings_shutdown(void)
{
    GSList *bindings, *l;
    guint n;
    EsconfGBinding *binding;
    ...
    if(G_UNLIKELY(__bindings)) {
        ...
        g_debug("%d esconf binding(s) are still connected. Are you sure all esconf "
                "channels are released before calling esconf_shutdown()?", n);
#endif
...
/**
 * esconf_g_property_bind:
 * @channel: An #EsconfChannel.
 * @esconf_property: A property on @channel.
 * @esconf_property_type: The type of @esconf_property.
 * @object: A #GObject.
 * @object_property: A valid property on @object.
 *
 * Binds an Esconf property to a #GObject property.  If the property
 * is changed via either the #GObject or Esconf, the corresponding
 * property will also be updated.
 *
 * Note that @esconf_property_type is required since @esconf_property
 * may or may not already exist in the Esconf store.  The type of
 ...
 **/
gulong
esconf_g_property_bind(EsconfChannel *channel,
                       const gchar *esconf_property,
                       GType esconf_property_type,
                       gpointer object,
                       const gchar *object_property)
{
    ...
    g_return_val_if_fail(ESCONF_IS_CHANNEL(channel), 0UL);
    g_return_val_if_fail(esconf_property && *esconf_property == '/', 0UL);
    g_return_val_if_fail(esconf_property_type != G_TYPE_NONE, 0UL);
    g_return_val_if_fail(esconf_property_type != G_TYPE_INVALID, 0UL);
    ...
    if(G_UNLIKELY(!g_value_type_transformable(esconf_property_type,
    ...
    return esconf_g_property_init(channel, esconf_property,
                                  esconf_property_type, G_OBJECT(object),
                                  object_property,
                                  G_PARAM_SPEC_VALUE_TYPE(pspec));
}

/**
 * esconf_g_property_bind_gdkcolor:
 * @channel: An #EsconfChannel.
 * @esconf_property: A property on @channel.
 * @object: A #GObject.
 * @object_property: A valid property on @object.
 *
 * Binds an Esconf property to a #GObject property of type
 ...
 * Esconf store as four 16-bit unsigned ints (red, green, blue, alpha).
 ...
 **/
gulong
esconf_g_property_bind_gdkcolor(EsconfChannel *channel,
                                const gchar *esconf_property,
                                gpointer object,
                                const gchar *object_property)
{
...
/**
 * esconf_g_property_bind_gdkrgba:
 * @channel: An #EsconfChannel.
 * @esconf_property: A property on @channel.
 * @object: A #GObject.
 * @object_property: A valid property on @object.
 ...
 **/
gulong
esconf_g_property_bind_gdkrgba(EsconfChannel *channel,
                               const gchar *esconf_property,
                               gpointer object,
                               const gchar *object_property)
{
...
/**
 * esconf_g_property_unbind:
 * @id: A binding ID number.
 *
 * Removes an Esconf/GObject property binding based on the binding
 * ID number.  See esconf_g_property_bind().
 **/
void
esconf_g_property_unbind(gulong id)
{
    GSList *l;
    EsconfGBinding *binding;
...
/**
 * esconf_g_property_unbind_by_property:
 * @channel: An #EsconfChannel.
 * @esconf_property: A bound property on @channel.
 * @object: A #GObject.
 * @object_property: A bound property on @object.
 *
 * Causes an Esconf channel previously bound to a #GObject property
 * (see esconf_g_property_bind()) to no longer be bound.
 **/
void
esconf_g_property_unbind_by_property(EsconfChannel *channel,
                                     const gchar *esconf_property,
                                     gpointer object,
                                     const gchar *object_property)
{
    ...
    g_return_if_fail(ESCONF_IS_CHANNEL(channel));
    g_return_if_fail(esconf_property && *esconf_property == '/');
    g_return_if_fail(G_IS_OBJECT(object));
    ...
    for(l = __bindings; l; l = g_slist_next(l)) {
        ...
        if(binding->object == object
           && binding->channel == channel
           && !strcmp(esconf_property, binding->esconf_property)
...
/**
 * esconf_g_property_unbind_all:
 * @channel_or_object: A #GObject or #EsconfChannel.
 *
 * Unbinds all Esconf channel bindings (see esconf_g_property_bind())
 * to @object.  If @object is an #EsconfChannel, it will unbind all
 * esconf properties on that channel.  If @object is a regular #GObject
 * with properties bound to a channel, all those bindings will be
 * removed.
 **/
void
esconf_g_property_unbind_all(gpointer channel_or_object)
{
    guint n;

    g_return_if_fail(G_IS_OBJECT(channel_or_object));

    if(ESCONF_IS_CHANNEL(channel_or_object)) {
        n = g_signal_handlers_disconnect_matched(channel_or_object, G_SIGNAL_MATCH_FUNC,
                                                 0, 0, NULL,
                                                 esconf_g_property_channel_notify,
                                                 NULL);
    } else {
        n = g_signal_handlers_disconnect_matched(channel_or_object, G_SIGNAL_MATCH_FUNC,
                                                 0, 0, NULL,
                                                 esconf_g_property_object_notify,
                                                 NULL);
    }

    if(G_UNLIKELY(!n)) {
        g_warning("No bindings were found on the %s",
                  ESCONF_IS_CHANNEL(channel_or_object) ? "channel" : "object");
    }
}



#define __ESCONF_BINDING_C__
