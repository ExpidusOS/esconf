...
/**
 * SECTION:esconf-channel
 * @title: Esconf Channel
 * @short_description: An application-defined domain for storing configuration settings
 *
 * An EsconfChannel is a representation of a restricted domain or
...
typedef struct _EsconfChannelClass
{
    GObjectClass parent;

    void (*property_changed)(EsconfChannel *channel,
...
static GObject *esconf_channel_constructor(GType type,
                                           guint n_construct_properties,
                                           GObjectConstructParam *construct_properties);
static void esconf_channel_set_g_property(GObject *object,
                                          guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void esconf_channel_get_g_property(GObject *object,
                                          guint property_id,
                                          GValue *value,
                                          GParamSpec *pspec);
static void esconf_channel_dispose(GObject *obj);
static void esconf_channel_finalize(GObject *obj);

static void esconf_channel_property_changed(EsconfCache *cache,
...
G_DEFINE_TYPE(EsconfChannel, esconf_channel, G_TYPE_OBJECT)


static void
esconf_channel_class_init(EsconfChannelClass *klass)
{
    GObjectClass *object_class = (GObjectClass *)klass;

    object_class->constructor = esconf_channel_constructor;
    object_class->set_property = esconf_channel_set_g_property;
    object_class->get_property = esconf_channel_get_g_property;
    object_class->dispose = esconf_channel_dispose;
    object_class->finalize = esconf_channel_finalize;

    /**
     * EsconfChannel::property-changed:
     * @channel: The #EsconfChannel emitting the signal.
    ...
    signals[SIG_PROPERTY_CHANGED] = g_signal_new(I_("property-changed"),
                                                 ...
                                                 G_SIGNAL_RUN_LAST
                                                 | G_SIGNAL_DETAILED,
                                                 G_STRUCT_OFFSET(EsconfChannelClass,
                                                                 property_changed),
                                                 NULL,
                                                 NULL,
                                                 _esconf_marshal_VOID__STRING_BOXED,
                                                 G_TYPE_NONE,
                                                 2, G_TYPE_STRING,
                                                 G_TYPE_VALUE);

    /**
     * EsconfChannel::channel-name:
     *
    ...
    /**
     * EsconfChannel::is-singleton:
     *
     * Identifies the instance of the class as a singleton instance
     * or not.  This is mainly used internally by #EsconfChannel
...
static void
esconf_channel_init(EsconfChannel *instance)
{
}

static GObject *
esconf_channel_constructor(GType type,
                           guint n_construct_properties,
                           GObjectConstructParam *construct_properties)
{
    ...
    if(is_singleton) {
        ...
        if(!channel) {
            channel = ESCONF_CHANNEL(G_OBJECT_CLASS(esconf_channel_parent_class)->constructor(type,
    ...
    if(!channel->cache) {
        channel->cache = esconf_cache_new(channel_name);
        esconf_cache_prefetch(channel->cache, channel->property_base, NULL);
        g_signal_connect(channel->cache, "property-changed",
                         G_CALLBACK(esconf_channel_property_changed), channel);
    }
...
static void
esconf_channel_set_g_property(GObject *object,
                              guint property_id,
                              const GValue *value,
                              GParamSpec *pspec)
{
    EsconfChannel *channel = ESCONF_CHANNEL(object);
...
static void
esconf_channel_get_g_property(GObject *object,
                              guint property_id,
                              GValue *value,
                              GParamSpec *pspec)
{
...
static void
esconf_channel_dispose(GObject *obj)
{
    EsconfChannel *channel = ESCONF_CHANNEL(obj);

    if(!channel->disposed) {
        channel->disposed = TRUE;

        g_signal_handlers_disconnect_by_func(channel->cache,
                                             esconf_channel_property_changed,
                                             channel);
        g_object_unref(G_OBJECT(channel->cache));
    }

    G_OBJECT_CLASS(esconf_channel_parent_class)->dispose(obj);
}

static void
esconf_channel_finalize(GObject *obj)
{
    EsconfChannel *channel = ESCONF_CHANNEL(obj);
    ...
    G_OBJECT_CLASS(esconf_channel_parent_class)->finalize(obj);
}



void
_esconf_channel_shutdown(void)
{
    G_LOCK(__singletons);
...
static void
esconf_channel_property_changed(EsconfCache *cache,
                                const gchar *channel_name,
                                const gchar *property,
                                const GValue *value,
                                gpointer user_data)
{
    EsconfChannel *channel = ESCONF_CHANNEL(user_data);
...
static gboolean
esconf_channel_set_internal(EsconfChannel *channel,
                            const gchar *property,
                            GValue *value)
{
    gboolean ret;
    ...
    g_return_val_if_fail(ESCONF_IS_CHANNEL(channel) && property, FALSE);

    ret = esconf_cache_set(channel->cache, real_property, value, ERROR);
    if(!ret)
        ERROR_CHECK;
...
static gboolean
esconf_channel_get_internal(EsconfChannel *channel,
                            const gchar *property,
                            GValue *value)
{
     ...
     * 1.  |value| is unset, and we just take the property from esconf
     *     and give it to the caller as-is, in the type esconf gives
    ...
    ret = esconf_cache_lookup(channel->cache, real_property, val, ERROR);
    if(!ret)
        ERROR_CHECK;
...
static GPtrArray *
esconf_transform_array(GPtrArray *arr_src,
                       GType gtype)
{
    GPtrArray *arr_dest;
    guint i;
    ...
    for(i = 0; i < arr_src->len; ++i) {
        ...
        if(G_VALUE_TYPE(value_src) == gtype)
            ...
            _esconf_gvalue_free(value_dest);
 ...
 *
 * The reference count of the returned channel is owned by libesconf.
 *
 * Returns: (transfer full): An #EsconfChannel singleton.
 *
 * Since: 4.5.91
 */
EsconfChannel *
esconf_channel_get(const gchar *channel_name)
{
    return g_object_new(ESCONF_TYPE_CHANNEL,
 ...
 * esconf_channel_get(), which returns a singleton object and
 * saves a little memory.  However, esconf_channel_new() can be
 * useful in some cases where you want to tie an #EsconfChannel's
 ...
 * so in this cases it is highly recommended to use esconf_channel_get().
 *
 * Returns: A new #EsconfChannel.  Release with g_object_unref()
 *          when no longer needed.
 **/
EsconfChannel *
esconf_channel_new(const gchar *channel_name)
{
    return g_object_new(ESCONF_TYPE_CHANNEL,
                        "channel-name", channel_name,
                        "is-singleton", FALSE,
...
/**
 * esconf_channel_new_with_property_base:
 * @channel_name: A channel name.
 ...
 **/
EsconfChannel *
esconf_channel_new_with_property_base(const gchar *channel_name,
...
/**
 * esconf_channel_has_property:
 * @channel: An #EsconfChannel.
 ...
 **/
gboolean
esconf_channel_has_property(EsconfChannel *channel,
                            const gchar *property)
{
    gboolean exists;
    gchar *real_property = REAL_PROP(channel, property);
    ...
    exists = esconf_cache_lookup(channel->cache, real_property, NULL, ERROR);
    if(!exists)
        ERROR_CHECK;
...
/**
 * esconf_channel_is_property_locked:
 * @channel: An #EsconfChannel.
 * @property: A property name.
 *
 * Queries whether or not @property on @channel is locked by system
 * policy.  If the property is locked, calls to
 * esconf_channel_set_property() (or any of the "set" family of functions)
 * or esconf_channel_reset_property() will fail.
 *
 ...
 **/
gboolean
esconf_channel_is_property_locked(EsconfChannel *channel,
                                  const gchar *property)
{
    ...
    if (!esconf_exported_call_is_property_locked_sync ((EsconfExported*)proxy, channel->channel_name,
 ...
 * Since Esconf backends are expected to support setting defaults
 ...
 **/
void
esconf_channel_reset_property(EsconfChannel *channel,
                              const gchar *property_base,
                              gboolean recursive)
{
    ...
    if(!esconf_cache_reset(channel->cache, real_property_base, recursive, ERROR))
        ERROR_CHECK;
 ...
 */
GHashTable *
esconf_channel_get_properties(EsconfChannel *channel,
                              const gchar *property_base)
{
    GDBusProxy *proxy = _esconf_get_gdbus_proxy ();
    ...
    if(!esconf_exported_call_get_all_properties_sync ((EsconfExported*)proxy, channel->channel_name,
                                                      real_property_base
                                                      ? real_property_base : "/",
                                                      &variant, NULL, ERROR))
    {
    ...
    if (variant) {
        properties = esconf_gvariant_to_hash (variant);
 ...
 **/
gchar *
esconf_channel_get_string(EsconfChannel *channel,
                          const gchar *property,
                          const gchar *default_value)
{
    ...
    g_return_val_if_fail(ESCONF_IS_CHANNEL(channel) && property, NULL);

    if(esconf_channel_get_internal(channel, property, &val)) {
...
/**
 * esconf_channel_get_string_list:
 * @channel: An #EsconfChannel.
 * @property: A property name.
 *
 ...
 */
gchar **
esconf_channel_get_string_list(EsconfChannel *channel,
                               const gchar *property)
{
    gchar **values = NULL;
    GPtrArray *arr;
    ...
    arr = esconf_channel_get_arrayv(channel, property);
    if(!arr)
        return NULL;
 ...
 **/
gint
esconf_channel_get_int(EsconfChannel *channel,
                       const gchar *property,
                       gint default_value)
{
    ...
    g_return_val_if_fail(ESCONF_IS_CHANNEL(channel) && property, value);
 ...
 **/
guint32
esconf_channel_get_uint(EsconfChannel *channel,
                        const gchar *property,
 ...
 **/
guint64
esconf_channel_get_uint64(EsconfChannel *channel,
                          const gchar *property,
 ...
 **/
gdouble
esconf_channel_get_double(EsconfChannel *channel,
                          const gchar *property,
 ...
 **/
gboolean
esconf_channel_get_bool(EsconfChannel *channel,
                        const gchar *property,
 ...
 **/
gboolean
esconf_channel_set_string(EsconfChannel *channel,
                          const gchar *property,
                          const gchar *value)
{
    GValue val = { 0, };
    ...
    ret = esconf_channel_set_internal(channel, property, &val);
...
/**
 * esconf_channel_set_string_list:
 * @channel: An #EsconfChannel.
 * @property: A property name.
 ...
 **/
gboolean
esconf_channel_set_string_list(EsconfChannel *channel,
                               const gchar *property,
                               const gchar * const *values)
{
    ...
    g_return_val_if_fail(ESCONF_IS_CHANNEL(channel) && property && values
                         && values[0], FALSE);
    ...
    ret = esconf_channel_set_arrayv(channel, property, arr);
 ...
 **/
gboolean
esconf_channel_set_int(EsconfChannel *channel,
                       const gchar *property,
                       gint value)
{
    GValue val = { 0, };
 ...
 **/
gboolean
esconf_channel_set_uint(EsconfChannel *channel,
                        const gchar *property,
                        guint32 value)
{
    GValue val = { 0, };
 ...
 **/
gboolean
esconf_channel_set_uint64(EsconfChannel *channel,
                          const gchar *property,
                          guint64 value)
{
    GValue val = { 0, };
 ...
 **/
gboolean
esconf_channel_set_double(EsconfChannel *channel,
                          const gchar *property,
                          gdouble value)
{
    GValue val = { 0, };
 ...
 **/
gboolean
esconf_channel_set_bool(EsconfChannel *channel,
                        const gchar *property,
                        gboolean value)
{
    GValue val = { 0, };
 ...
 * @value.  If @value is initialized to a particular type, libesconf
 ...
 **/
gboolean
esconf_channel_get_property(EsconfChannel *channel,
                            const gchar *property,
                            GValue *value)
{
    GValue val1 = { 0, };
    ...
    ret = esconf_channel_get_internal(channel, property, &val1);
    ...
    if(ret) {
           ...
           && G_VALUE_TYPE(value) != G_VALUE_TYPE(&val1))
        {
            ...
            if(G_VALUE_TYPE(&val1) == G_TYPE_PTR_ARRAY) {
                ...
                GPtrArray *arr = esconf_transform_array(g_value_get_boxed(&val1),
                                                        G_VALUE_TYPE(value));
 ...
 **/
gboolean
esconf_channel_set_property(EsconfChannel *channel,
                            const gchar *property,
                            const GValue *value)
{
    GValue val = { 0, };
    ...
    g_return_val_if_fail(ESCONF_IS_CHANNEL(channel)
                         && property
 ...
 **/
gboolean
esconf_channel_get_array(EsconfChannel *channel,
                         const gchar *property,
                         ...
                         ...)
{
    ...
    ret = esconf_channel_get_array_valist(channel, property, first_value_type,
                                          var_args);
    va_end(var_args);
...
/**
 * esconf_channel_get_array_valist:
 * @channel: An #EsconfChannel.
 * @property: A property string.
 * @first_value_type: The type of the first argument in the array.
 * @var_args: A variable argument list of types and values.
 *
 * Gets an array property on @channel.  See esconf_channel_get_array()
 * for details.
 *
 * Returns: %TRUE if the property was retrieved successfully,
 *          %FALSE otherwise.
 **/
gboolean
esconf_channel_get_array_valist(EsconfChannel *channel,
                                const gchar *property,
                                GType first_value_type,
                                va_list var_args)
{
        ...
        cur_value_type = va_arg(var_args, GType), ++i)
    {
        ...
        if(G_VALUE_TYPE(val) != cur_value_type
           && !((G_VALUE_TYPE(val) == G_TYPE_UINT && cur_value_type == ESCONF_TYPE_UINT16)
                || (G_VALUE_TYPE(val) == G_TYPE_INT && cur_value_type == ESCONF_TYPE_INT16)))
        {
#ifdef ESCONF_ENABLE_CHECKS
...
/**
 * esconf_channel_get_arrayv:
 * @channel: An #EsconfChannel.
 * @property: A property string.
 *
 * Gets an array property on @channel and returns it as
 * a #GPtrArray, which can be freed with esconf_array_free()
 * when no longer needed.
 *
 ...
 **/
GPtrArray *
esconf_channel_get_arrayv(EsconfChannel *channel,
                          const gchar *property)
{
    GValue val = { 0, };
    GPtrArray *arr = NULL;
    ...
    ret = esconf_channel_get_internal(channel, property, &val);
 ...
 **/
gboolean
esconf_channel_set_array(EsconfChannel *channel,
                         const gchar *property,
                         GType first_value_type,
                         ...)
{
    ...
    va_start(var_args, first_value_type);
    ret = esconf_channel_set_array_valist(channel, property, first_value_type,
                                          var_args);
    va_end(var_args);
...
/**
 * esconf_channel_set_array_valist:
 * @channel: An #EsconfChannel.
 * @property: A property string.
 * @first_value_type: The type of the first argument in the array.
 * @var_args: A variable argument list of types and values.
 *
 * Sets an array property on @channel.  See esconf_channel_set_array()
 * for details.
 *
 * Returns: %TRUE if the property was set successfully, %FALSE otherwise.
 **/
gboolean
esconf_channel_set_array_valist(EsconfChannel *channel,
                                const gchar *property,
                                GType first_value_type,
                                va_list var_args)
{
    ...
    g_return_val_if_fail(ESCONF_IS_CHANNEL(channel) && property
 ...
 **/
gboolean
esconf_channel_set_arrayv(EsconfChannel *channel,
                          const gchar *property,
                          GPtrArray *values)
{
    GValue val = { 0, };
...
/**
 * esconf_channel_get_named_struct:
 * @channel: An #EsconfChannel.
 * @property: A string property name.
 ...
 * with esconf_named_struct_register().
 *
 * Returns: %TRUE if the property was retrieved successfully,
 *          %FALSE otherwise.
 **/
gboolean
esconf_channel_get_named_struct(EsconfChannel *channel,
                                const gchar *property,
                                const gchar *struct_name,
                                gpointer value_struct)
{
    EsconfNamedStruct *ns = _esconf_named_struct_lookup(struct_name);

    if(!ns)
        return FALSE;

    return esconf_channel_get_structv(channel, property, value_struct,
                                      ns->n_members, ns->member_types);
}

/**
 * esconf_channel_set_named_struct:
 * @channel: An #EsconfChannel.
 * @property: A string property name.
 * @struct_name: A string struct name.
 * @value_struct: A pointer to a struct.
 ...
 **/
gboolean
esconf_channel_set_named_struct(EsconfChannel *channel,
                                const gchar *property,
                                const gchar *struct_name,
                                gpointer value_struct)
{
    ...
    return esconf_channel_set_structv(channel, property, value_struct,
                                      ns->n_members, ns->member_types);
}


/**
 * esconf_channel_get_struct:
 * @channel: An #EsconfChannel.
 * @property: A string property name.
 ...
 **/
gboolean
esconf_channel_get_struct(EsconfChannel *channel,
                          const gchar *property,
                          gpointer value_struct,
                          ...
                          ...)
{
    ...
    ret = esconf_channel_get_struct_valist(channel, property, value_struct,
...
/**
 * esconf_channel_get_struct_valist:
 * @channel: An #EsconfChannel.
 * @property: A string property name.
 * @value_struct: A pointer to a struct in which to store values.
 * @first_member_type: The GType of the first member of @value_struct.
 ...
 *
 * Gets a property on @channel and stores it as members of the
 * @value_struct struct.  See esconf_channel_get_struct() for details.
 *
 * Note: Struct members can only be non-pointer types such as int,
 * boolean, double, etc.
 *
 ...
 **/
gboolean
esconf_channel_get_struct_valist(EsconfChannel *channel,
                                 const gchar *property,
                                 gpointer value_struct,
                                 GType first_member_type,
                                 va_list var_args)
{
    ...
    g_return_val_if_fail(ESCONF_IS_CHANNEL(channel) && property && value_struct
    ...
    ret = esconf_channel_get_structv(channel, property, value_struct,
 ...
 **/
gboolean
esconf_channel_get_structv(EsconfChannel *channel,
                           const gchar *property,
                           gpointer value_struct,
                           guint n_members,
                           GType *member_types)
    ...
    *__val_p = cvalgetter(val); \
    cur_offset += sizeof(ctype); \
        ...
        switch(member_types[i]) {
            ...
            default:
                if(ESCONF_TYPE_UINT16 == member_types[i]) {
                    /* uint16 is stored as uint */
                    SET_STRUCT_VAL(guint16, G_TYPE_UINT,
                                   ALIGNOF_GUINT16, g_value_get_uint);
                } else if(ESCONF_TYPE_INT16 == member_types[i]) {
                    /* int16 is stored as int */
 ...
 **/
gboolean
esconf_channel_set_struct(EsconfChannel *channel,
                          const gchar *property,
                          ...
                          ...)
{
    ...
    va_start(var_args, first_member_type);
    ret = esconf_channel_set_struct_valist(channel, property, value_struct,
                                           first_member_type, var_args);
    va_end(var_args);
...
/**
 * esconf_channel_set_struct_valist:
 * @channel: An #EsconfChannel.
 * @property: A string property name.
 * @value_struct: A pointer to a struct from which to take values.
 * @first_member_type: The GType of the first member of @value_struct.
 ...
 *
 * Sets a property on @channel using the members of @value_struct
 * as a value array.  See esconf_channel_set_struct() for details.
 *
 * Returns: %TRUE if the property was set successfully,
 *          %FALSE oherwise.
 **/
gboolean
esconf_channel_set_struct_valist(EsconfChannel *channel,
                                 const gchar *property,
                                 const gpointer value_struct,
                                 GType first_member_type,
                                 va_list var_args)
{
    ...
    ret = esconf_channel_set_structv(channel, property, value_struct,
                                     n_members, member_types);
    g_free(member_types);
 ...
 **/
gboolean
esconf_channel_set_structv(EsconfChannel *channel,
                           const gchar *property,
                           const gpointer value_struct,
                           guint n_members,
                           GType *member_types)
        ...
        switch(member_types[i]) {
            ...
            default:
                if(ESCONF_TYPE_UINT16 == member_types[i]) {
                    ...
                    GET_STRUCT_VAL(guint16, ESCONF_TYPE_UINT16,
                                   ALIGNOF_GUINT16, esconf_g_value_set_uint16);
                } else if(ESCONF_TYPE_INT16 == member_types[i]) {
                    /* _set_arrayv() will convert these */
                    GET_STRUCT_VAL(gint16, ESCONF_TYPE_INT16,
                                   ALIGNOF_GINT16, esconf_g_value_set_int16);
                } else {
#ifdef ESCONF_ENABLE_CHECKS
                    g_warning("Unable to handle value type %ld (%s) when " \
...
/**
 * esconf_list_channels:
 *
 * Lists all channels known in the Esconf configuration store.
 *
 ...
 */
gchar **
esconf_list_channels(void)
{
    GDBusProxy *proxy = _esconf_get_gdbus_proxy();
    gchar **channels = NULL;
    ERROR_DEFINE;

    if(!esconf_exported_call_list_channels_sync ((EsconfExported*)proxy, 
                                               &channels, NULL, ERROR))
        ERROR_CHECK;
