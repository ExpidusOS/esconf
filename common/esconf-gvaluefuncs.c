...
#include "esconf-gvaluefuncs.h"
#include "esconf/esconf-types.h"
#include "esconf-common-private.h"

#ifdef CHAR_MIN
#define ESCONF_MINCHAR  CHAR_MIN
#else
#define ESCONF_MINCHAR  (-128)
#endif

#ifdef CHAR_MAX
#define ESCONF_MAXCHAR  CHAR_MAX
#else
#define ESCONF_MAXCHAR  (127)
#endif

#ifdef UCHAR_MAX
#define ESCONF_MAXUCHAR  UCHAR_MAX
#else
#define ESCONF_MAXUCHAR  (255)
#endif

GType
_esconf_gtype_from_string(const gchar *type)
{
    ...
    if(!strcmp(type, "empty"))
        ...
        return ESCONF_TYPE_UINT16;
    else if(!strcmp(type, "int16"))
        return ESCONF_TYPE_INT16;
...
const gchar *
_esconf_string_from_gtype(GType gtype)
{
    switch(gtype) {
        case G_TYPE_STRING:
            return "string";
        ...
        case G_TYPE_BOOLEAN:
            return "bool";
        default:
            if(gtype == ESCONF_TYPE_UINT16)
                return "uint16";
            else if(gtype == ESCONF_TYPE_INT16)
                return "int16";
    ...
    g_warning("GType %s doesn't map to an Esconf type",
              g_type_name(gtype));
    return NULL;
}

gboolean
_esconf_gvalue_from_string(GValue *value,
                           const gchar *str)
{
...
#define HANDLE_INT(minval, maxval, setfunc)  REAL_HANDLE_INT(minval, maxval, strtol, setfunc)
    ...
    switch(G_VALUE_TYPE(value)) {
        ...
        case G_TYPE_UCHAR:
            HANDLE_UINT(0, ESCONF_MAXUCHAR, g_value_set_uchar);
        case G_TYPE_CHAR:
        ...
        default:
            if(ESCONF_TYPE_UINT16 == G_VALUE_TYPE(value)) {
                HANDLE_INT(0, G_MAXUSHORT, esconf_g_value_set_uint16);
                return TRUE;
            } else if(ESCONF_TYPE_INT16 == G_VALUE_TYPE(value)) {
                HANDLE_INT(G_MINSHORT, G_MAXSHORT, esconf_g_value_set_int16);
                return TRUE;
...
gchar *
_esconf_string_from_gvalue(GValue *val)
{
    ...
    switch(G_VALUE_TYPE(val)) {
        ...
        default:
            if(G_VALUE_TYPE(val) == ESCONF_TYPE_UINT16) {
                return g_strdup_printf("%u",
                                       (guint)esconf_g_value_get_uint16(val));
            } else if(G_VALUE_TYPE(val) == ESCONF_TYPE_INT16) {
                return g_strdup_printf("%d",
                                       (gint)esconf_g_value_get_int16(val));
            }
            break;
    }
...
gboolean
_esconf_gvalue_is_equal(const GValue *value1,
                        const GValue *value2)
{
...
#define HANDLE_CMP_GV(TYPE, getter) \
        ...
        default:
            if(G_VALUE_TYPE(value1) == ESCONF_TYPE_INT16)
                return esconf_g_value_get_int16(value1) == esconf_g_value_get_uint16(value2);
            else if(G_VALUE_TYPE(value1) == ESCONF_TYPE_UINT16)
                return esconf_g_value_get_uint16(value1) == esconf_g_value_get_uint16(value2);
            break;
#undef HANDLE_CMP_GV
    }
...
void
_esconf_gvalue_free(GValue *value)
{
    if(!value)
        return;
    g_value_unset(value);
...
GVariant *
esconf_basic_gvalue_to_gvariant (const GValue *value) {
    ...
    case G_TYPE_STRING:
        ...
        break;
    default:
        if (G_VALUE_TYPE(value) == ESCONF_TYPE_INT16)
            type = G_VARIANT_TYPE_INT16;
        else if (G_VALUE_TYPE(value) == ESCONF_TYPE_UINT16)
    ...
    g_warning ("Unable to convert GType '%s' to GVariant", _esconf_string_from_gtype(G_VALUE_TYPE(value)));

    return NULL;
}

gboolean esconf_basic_gvariant_to_gvalue (GVariant *variant, GValue *value)
{
  gboolean ret = TRUE;
...
GVariant *
esconf_gvalue_to_gvariant (const GValue *value)
{
    ...
    if (G_VALUE_TYPE(value) == G_TYPE_PTR_ARRAY) {
        ...
        if (arr->len == 0) {
            ...
            for (i=0; i < arr->len; ++i) {
                ...
                var = esconf_basic_gvalue_to_gvariant (v);
                if (var) {
        ...
        variant = esconf_basic_gvalue_to_gvariant(value);

    return variant;
}

GVariant *esconf_hash_to_gvariant (GHashTable *hash)
{
    GHashTableIter iter;
    GVariantBuilder builder;
    GVariant *variant;
    ...
    while (g_hash_table_iter_next (&iter, (gpointer)&key, (gpointer)&value)) {
        ...
        if (G_VALUE_TYPE (value) == G_TYPE_PTR_ARRAY) {
            ...
            for (i = 0; i < arr->len; ++i) {
                ...
                if(item_value)
                {
                    v = esconf_basic_gvalue_to_gvariant(item_value);
                    if (v) {
            ...
            v = esconf_basic_gvalue_to_gvariant(value);
            if (v) {
                g_variant_builder_add (&builder, "{sv}", key, v);
                g_variant_unref (v);
            }
...
GPtrArray *
esconf_dup_value_array (GPtrArray *arr, gboolean auto_destroy_value) {
...
GValue * esconf_gvariant_to_gvalue (GVariant *in_variant)
{
    GValue *value ;
    GVariant *variant;
    ...
    if (g_variant_is_of_type (variant, G_VARIANT_TYPE("av"))) {
        ...
        while (idx < nchild ) {
            ...
            esconf_basic_gvariant_to_gvalue (v, arr_val);
      ...
      if (!esconf_basic_gvariant_to_gvalue(variant, value)) {
        g_free(value);
        return NULL;
      }
    }
...
GHashTable *esconf_gvariant_to_hash (GVariant *variant)
{
    ...
    while (g_variant_iter_next (&iter, "{sv}", &key, &v)) {
        ...
        value = esconf_gvariant_to_gvalue (v);
