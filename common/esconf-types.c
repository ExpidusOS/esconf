...
/**
 * SECTION:esconf-types
 * @title: Esconf Types
 * @short_description: GObject types used by the Esconf daemon and library
 *
 *  libgobject lacks GObject fundamental types for 16-bit signed and unsigned integers, which may be useful to use in an Esconf store. GObject types for these primitive types are provided here.
 **/
...
#define HANDLE_TYPE(gtype_s, getter) \
    ...
    if(G_VALUE_TYPE(dest_value) == ESCONF_TYPE_UINT16) {
        if(dest > USHRT_MAX) {
            g_warning("Converting type \"%s\" to \"%s\" results in overflow",
                      G_VALUE_TYPE_NAME(src_value),
                      G_VALUE_TYPE_NAME(dest_value));
        }
        esconf_g_value_set_uint16(dest_value, (guint16)dest);
    } else if(G_VALUE_TYPE(dest_value) == ESCONF_TYPE_INT16) {
        if(dest > (guint64) SHRT_MAX || dest < (guint64) SHRT_MIN) {
            g_warning("Converting type \"%s\" to \"%s\" results in overflow",
                      G_VALUE_TYPE_NAME(src_value),
                      G_VALUE_TYPE_NAME(dest_value));
        }
        esconf_g_value_set_int16(dest_value, (gint16)dest);
    }
#undef HANDLE_TYPE
}
...
#define HANDLE_TYPE(gtype_s, setter) \
    ...
    if(G_VALUE_TYPE(src_value) == ESCONF_TYPE_UINT16)
        src = esconf_g_value_get_uint16(src_value);
    else if(G_VALUE_TYPE(src_value) == ESCONF_TYPE_INT16) {
        src = esconf_g_value_get_int16(src_value);
        is_signed = TRUE;
    } else
        return;
 ...
 **/
guint16
esconf_g_value_get_uint16(const GValue *value)
{
    g_return_val_if_fail(G_VALUE_HOLDS(value, ESCONF_TYPE_UINT16), 0);
    return (guint16)value->data[0].v_int;
}

/**
 * esconf_g_value_set_uint16:
 * @value: A #GValue.
 * @v_uint16: A guint16.
 *
 ...
 **/
void
esconf_g_value_set_uint16(GValue *value,
                          guint16 v_uint16)
{
    g_return_if_fail(G_VALUE_HOLDS(value, ESCONF_TYPE_UINT16));
    value->data[0].v_int = v_uint16;
}

GType
esconf_int16_get_type(void)
{
 ...
 **/
gint16
esconf_g_value_get_int16(const GValue *value)
{
    g_return_val_if_fail(G_VALUE_HOLDS(value, ESCONF_TYPE_INT16), 0);
    return (gint16)value->data[0].v_int;
}

/**
 * esconf_g_value_set_int16:
 * @value: A #GValue.
 * @v_int16: A gint16.
 *
 ...
 **/
void
esconf_g_value_set_int16(GValue *value,
                         gint16 v_int16)
{
    g_return_if_fail(G_VALUE_HOLDS(value, ESCONF_TYPE_INT16));
    value->data[0].v_int = v_int16;
}


#ifdef LIBESCONF_COMPILATION
#define __ESCONF_TYPES_C__
